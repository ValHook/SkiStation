...
domain_t *create_domain(const char *filename)
{
        ...
        for (i = 0; i < nv->length; i++) {
                ...
                fscanf_ex(4, fp, "%*d%*[ \t\n]%50s%*[ \t\n]%d%*[ \t\n]%d%*[ \t\n]%d\n",
                          buf, &nv->items[i]->altitude, &nv->items[i]->pos_x, &nv->items[i]->pos_y);
        ...
        for (i = 0; i < ev->length; i++) {
                ...
                fscanf_ex(7, fp, "%*d%*[ \t\n]%50s%*[ \t\n]%4s%*[ \t\n]%d%*[ \t\n]%d%*[ \t\n]%d%*[ \t\n]%d%*[ \t\n]%d\n",
                          buf, buf_2, &ev->items[i]->parent, &ev->items[i]->child, &ev->items[i]->awesomeness, &ev->items[i]->pos_x, &ev->items[i]->pos_y);
                ...
                for (j = 0; j < NUMBER_OF_EDGE_TYPES; j++) {
                        if (strcmp(buf_2, edge_type[j].str) == 0) {
                                ...
                                if (ev->items[i]->type != BUS)
                                        ...
                                        /* ARC 1800-2000 */
                                        case 200: ev->items[i]->time = edge_type[11].route_time;
                                                break;
                                        /* ARC 1600-2000 */
                                        case 400: ev->items[i]->time = edge_type[11].wait_time;
                                                break;
...
{
        unsigned int i, remaining, shortest, n1 = 0, n2 = 0, weight = 0;
        edge_vector *ev, *r_ev;
